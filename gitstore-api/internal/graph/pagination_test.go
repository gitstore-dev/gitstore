...
func TestPaginateProducts(t *testing.T) {
	...
	t.Run("should paginate forward with first", func(t *testing.T) {
		...
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "A"), conn.Edges[0].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "B"), conn.Edges[1].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "C"), conn.Edges[2].Node.ID)
	...
	t.Run("should paginate forward with first and after", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, 3, len(conn.Edges))
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "D"), conn.Edges[0].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "E"), conn.Edges[1].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "F"), conn.Edges[2].Node.ID)
		assert.True(t, conn.PageInfo.HasNextPage)
	...
	t.Run("should paginate backward with last", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, 3, len(conn.Edges))
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "H"), conn.Edges[0].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "I"), conn.Edges[1].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "J"), conn.Edges[2].Node.ID)
		assert.False(t, conn.PageInfo.HasNextPage)
		assert.True(t, conn.PageInfo.HasPreviousPage)
	})
	...
	t.Run("should paginate backward with last and before", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, 3, len(conn.Edges))
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "E"), conn.Edges[0].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "F"), conn.Edges[1].Node.ID)
		assert.Equal(t, mustEncodeNodeID(nodeKindProduct, "G"), conn.Edges[2].Node.ID)
		assert.True(t, conn.PageInfo.HasNextPage)
		assert.True(t, conn.PageInfo.HasPreviousPage)
	})
